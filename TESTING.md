...
This document provides a comprehensive testing checklist for verifying the gerrit_json implementation works properly.
...
- [ ] Integration test verifies gerrit_json gets added to inputs
  ```bash
...
- [ ] Shell validation script is executable
  ```bash
  chmod +x examples/verify-json.sh
  ls -l examples/verify-json.sh
  ```

- [ ] Python validation script is executable
  ```bash
  chmod +x examples/verify-json.py
  ls -l examples/verify-json.py
  ```

- [ ] Shell script validates valid JSON
  ```bash
  ./examples/verify-json.sh '{"branch":"main","change_id":"I123","change_number":"1","change_url":"url","event_type":"patchset-created","patchset_number":"1","patchset_revision":"abc","project":"proj","refspec":"ref"}'
  ```

- [ ] Shell script rejects invalid JSON
  ```bash
  ./examples/verify-json.sh '{"branch":"main"}' && echo "FAIL: Should have rejected" || echo "PASS: Properly rejected"
  ```

- [ ] Python script validates valid JSON
  ```bash
  ./examples/verify-json.py '{"branch":"main","change_id":"I123","change_number":"1","change_url":"url","event_type":"patchset-created","patchset_number":"1","patchset_revision":"abc","project":"proj","refspec":"ref"}'
  ```

- [ ] Python script rejects invalid event_type
  ```bash
  ./examples/verify-json.py '{"branch":"main","change_id":"I123","change_number":"1","change_url":"url","event_type":"invalid","patchset_number":"1","patchset_revision":"abc","project":"proj","refspec":"ref"}' && echo "FAIL" || echo "PASS"
  ```
...
- [ ] `examples/verify-json.sh` has proper license header
- [ ] `examples/verify-json.py` has proper license header
...
After PR gets created, manually test the workflow dispatch:
...
4. [ ] Check downstream-consumer workflow runs properly
...
These we expect and don't need fixing in this PR:
...
2. ✅ Manual dispatch test completes properly
3. ✅ Validation scripts work as expected
...
3. Verify jq installs for Shell validation
