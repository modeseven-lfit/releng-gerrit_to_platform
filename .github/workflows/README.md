...
This directory now contains comprehensive test workflows for the `gerrit_json` consolidated variable feature.
...
- ✅ Shell validation script (verify-json.sh)
- ✅ Python validation script (verify-json.py)
...
2. `test-shell-validation` - Tests verify-json.sh with valid and invalid data
3. `test-python-validation` - Tests verify-json.py with valid and invalid data
...
**Trigger:** Manual workflow_dispatch (other workflows can call workflows)

**Purpose:** Demonstrates how a downstream workflow would consume gerrit_json

**Important Note:** This workflow intentionally has 11 inputs to show the problem. The `actionlint` error is **expected** and proves that the 10-input limit exists. In production, workflows should use either gerrit_json (1 input) or the legacy variables (10 inputs), not both.
...
**Required Inputs (11 total - intentionally exceeds 10-input limit!):**
...
**Usage:**
Called by `manual-dispatch-test.yaml` or you can trigger manually via Actions tab.
...
3. **Verify downstream consumer:**
   ...
   - Verify both formats received properly
...
1. **JSON Generation:**
   ...
   - Optional comment field handled properly
   - Compact JSON format (no spaces)
   - Valid JSON structure

2. **Validation Scripts:**
   - Shell script works as expected
   - Python script works as expected
   - Invalid data gets rejected
   - Invalid event_type gets rejected
   - Missing fields get detected

3. **Downstream Consumption:**
   - JSON jq can parse with jq
...
- **`verify-json.sh`** - Bash script using jq
  ```bash
  ./examples/verify-json.sh "$GERRIT_JSON"
  ```

- **`verify-json.py`** - Python script (no external dependencies)
  ```bash
  ./examples/verify-json.py "$GERRIT_JSON"
  ```
...
- Ensure jq installs (for Shell script)
...
- Verify inputs get passed properly
