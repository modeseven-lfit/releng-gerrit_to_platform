...
##############################################################################
"""
Verify gerrit_json against the schema.

Usage: verify-json.py <gerrit_json_string>

Example:
    ./verify-json.py '{"branch":"master","change_id":"I123...",...}'
"""
...
def verify_gerrit_json(gerrit_json_str: str) -> bool:
    """
    Verify gerrit_json against schema.

    Args:
        gerrit_json_str: JSON string to verify
    ...
    for key, value in data.items():
        if not isinstance(value, str):
            print(f"ERROR: Field '{key}' must be a string, got {type(value).__name__}")
            return False
    ...
    if unknown_fields:
        print(f"\nWarning: Unknown fields present: " f"{', '.join(unknown_fields)}")
...
def main() -> int:
    ...
    if len(sys.argv) != 2:
        ...
        print("\nExample:")
        print(f'  {sys.argv[0]} \'{{"branch":"master","change_id":"I123..."}}\'')
        return 1

    if not verify_gerrit_json(sys.argv[1]):
        return 1
