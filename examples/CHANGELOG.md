...
### Overview
Added a new `gerrit_json` input variable that consolidates all individual `GERRIT_*` variables into a single JSON object. This change addresses the limitation on the number of inputs that `workflow_dispatch` events accept in GitHub Actions.
...
- Modified `test_find_and_dispatch()` to verify `gerrit_json` gets added to inputs
...
- New `gerrit_json` variable appears alongside them
...
│   ├── README.md (new)
│   ├── verify-json.sh (new, executable)
│   └── verify-json.py (new, executable)
...
4. Deprecate individual `GERRIT_*` variables (future release)
